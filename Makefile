...
install:
	...
	go install github.com/18721889353/sunshine/cmd/protoc-gen-go-gin@latest
...
.PHONY: patch
# Patch some dependent code, e.g. make patch TYPE=types-pb , make patch TYPE=init-<your_db_driver>, your_db_driver is mysql, postgresql, tidb, sqlite, for example: make patch TYPE=init-mysql
patch:
