...
func (c *Client) Close() error {
	if atomic.CompareAndSwapInt32(&c.closed, 0, 1) {
		...
		close(c.closeCh)           // 通知监听 closeCh 的所有协程退出
		close(c.writeCh)           // 触发 writeLoop 的 ok=false 分支退出
		closeErr := c.conn.Close() // 关闭底层连接，触发 readLoop 的 ReadMessage 返回错误
		c.wg.Wait()                // 等待 writeLoop 和 readLoop 完全退出（readLoop 的 defer 会关闭 readCh）
		return closeErr
	}
	return nil
}
