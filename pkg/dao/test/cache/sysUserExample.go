...
const (
	// SysUserExampleCachePrefixKeyLock 分布式锁的 Redis key 前缀，必须以冒号结尾
	SysUserExampleCachePrefixKeyLock = "lock:sysUserExample:"
	// SysUserExampleCachePrefixKey 业务缓存数据的 Redis key 前缀，必须以冒号结尾
	SysUserExampleCachePrefixKey = "data:sysUserExample:"
	// SysUserExampleExpireTime 缓存数据的默认过期时间
	SysUserExampleExpireTime = 30 * time.Minute
)

// 确保 sysUserExampleCache 实现了 SysUserExampleCache 接口
var _ SysUserExampleCache = (*sysUserExampleCache)(nil)

// SysUserExampleCache 定义了针对 SysUserExample 模型的缓存操作接口
...
type SysUserExampleCache interface {
	...
	Set(ctx context.Context, id uint64, data *model.SysUserExample, duration time.Duration) error
	...
	// Get 根据 id 获取单个对象缓存
	Get(ctx context.Context, id uint64) (*model.SysUserExample, error)
	...
	// MultiGet 批量根据 id 列表获取对象缓存，返回 map[id]*SysUserExample
	MultiGet(ctx context.Context, ids []uint64) (map[uint64]*model.SysUserExample, error)

	// MultiSet 批量设置对象缓存，key 由每个对象的 ID 生成
	MultiSet(ctx context.Context, data []*model.SysUserExample, duration time.Duration) error
...
// sysUserExampleCache 是 SysUserExampleCache 接口的实现
type sysUserExampleCache struct {
	cache cache.Cache // 底层缓存驱动（如 Redis）
}

// NewSysUserExampleCache 创建一个新的 SysUserExampleCache 实例
...
func NewSysUserExampleCache(cacheType *database.CacheType) SysUserExampleCache {
	...
	if cType == consts.CacheTypeRedis {
		c := cache.NewRedisCache(cacheType.Rdb, cachePrefix, jsonEncoding, func() interface{} {
			return &model.SysUserExample{}
		})
		return &sysUserExampleCache{cache: c}
	}

	return nil // 无缓存
}

// GetSysUserExampleCacheKey 根据 id 生成完整的缓存 key（包含前缀）
func (c *sysUserExampleCache) GetSysUserExampleCacheKey(id uint64) string {
	return SysUserExampleCachePrefixKey + utils.Uint64ToStr(id)
}

// GetSysUserExampleCacheKeyString 根据字符串 key 生成完整的缓存 key（包含前缀）
func (c *sysUserExampleCache) GetSysUserExampleCacheKeyString(key string) string {
	return SysUserExampleCachePrefixKey + key
}

// getLockCacheKey 生成分布式锁的完整 key（包含锁前缀）
func (c *sysUserExampleCache) getLockCacheKey(key string) string {
	return fmt.Sprintf("%s%v", SysUserExampleCachePrefixKeyLock, key)
}

// GetLoopLock 获取阻塞式分布式锁（透传到底层缓存驱动）
func (c *sysUserExampleCache) GetLoopLock(ctx context.Context, key string, options ...redsync.Option) (*redsync.Mutex, error) {
	cacheKey := c.getLockCacheKey(key)
	return c.cache.GetLoopLock(ctx, cacheKey, options...)
}

// GetLock 获取非阻塞式分布式锁（透传到底层缓存驱动）
func (c *sysUserExampleCache) GetLock(ctx context.Context, key string, options ...redsync.Option) (*redsync.Mutex, error) {
	cacheKey := c.getLockCacheKey(key)
...
func (c *sysUserExampleCache) WatchDogLock(ctx context.Context, key string, expiry time.Duration, task func(ctx context.Context) error, options ...redsync.Option) error {
	// 1. 获取普通锁
...
func (c *sysUserExampleCache) WatchDogLoopLock(ctx context.Context, key string, expiry time.Duration, task func(ctx context.Context) error, options ...redsync.Option) error {
...
func (c *sysUserExampleCache) Set(ctx context.Context, id uint64, data *model.SysUserExample, duration time.Duration) error {
	if data == nil || id == 0 {
		return nil
	}
	cacheKey := c.GetSysUserExampleCacheKey(id)
...
// SetIDByKey 按自定义 key 缓存一个 uint64 类型的 ID
// key 不能为空，id 不能为 0
func (c *sysUserExampleCache) SetIDByKey(ctx context.Context, key string, id uint64, duration time.Duration) error {
	if key == "" || id == 0 {
		return nil
	}
	cacheKey := c.GetSysUserExampleCacheKeyString(key)
...
func (c *sysUserExampleCache) SetIDsByKey(ctx context.Context, key string, ids []uint64, duration time.Duration) error {
	if key == "" || ids == nil {
		return nil
	}
	cacheKey := c.GetSysUserExampleCacheKeyString(key)
...
func (c *sysUserExampleCache) Get(ctx context.Context, id uint64) (*model.SysUserExample, error) {
	var data *model.SysUserExample
	cacheKey := c.GetSysUserExampleCacheKey(id)
...
// GetIDByKey 根据自定义 key 获取缓存的 uint64 类型 ID
// 如果缓存不存在，返回 0 和错误
func (c *sysUserExampleCache) GetIDByKey(ctx context.Context, key string) (id uint64, err error) {
	cacheKey := c.GetSysUserExampleCacheKeyString(key)
...
// GetIDsByKey 根据自定义 key 获取缓存的 uint64 切片
// 如果缓存不存在，返回 nil 和错误
func (c *sysUserExampleCache) GetIDsByKey(ctx context.Context, key string) (ids []uint64, err error) {
	cacheKey := c.GetSysUserExampleCacheKeyString(key)
...
func (c *sysUserExampleCache) MultiSet(ctx context.Context, data []*model.SysUserExample, duration time.Duration) error {
	valMap := make(map[string]interface{})
	for _, v := range data {
		cacheKey := c.GetSysUserExampleCacheKey(v.ID)
		valMap[cacheKey] = v
	}
...
// 返回 map[id]*SysUserExample，只包含成功获取的项
// 如果某个 id 缓存缺失，则不会出现在返回 map 中
func (c *sysUserExampleCache) MultiGet(ctx context.Context, ids []uint64) (map[uint64]*model.SysUserExample, error) {
	var keys []string
	for _, v := range ids {
		cacheKey := c.GetSysUserExampleCacheKey(v)
		keys = append(keys, cacheKey)
	}

	itemMap := make(map[string]*model.SysUserExample)
	err := c.cache.MultiGet(ctx, keys, itemMap)
	if err != nil {
		return nil, err
	}

	retMap := make(map[uint64]*model.SysUserExample)
	for _, id := range ids {
		val, ok := itemMap[c.GetSysUserExampleCacheKey(id)]
		if ok {
			retMap[id] = val
		}
	}
...
// Del 根据 id 删除单个对象缓存
func (c *sysUserExampleCache) Del(ctx context.Context, id uint64) error {
	cacheKey := c.GetSysUserExampleCacheKey(id)
...
func (c *sysUserExampleCache) DelByPrefix(ctx context.Context, prefix string) error {
...
// DelByKey 根据自定义 key 删除缓存
func (c *sysUserExampleCache) DelByKey(ctx context.Context, key string) error {
	cacheKey := c.GetSysUserExampleCacheKeyString(key)
	err := c.cache.Del(ctx, cacheKey)
	if err != nil {
		return err
...
func (c *sysUserExampleCache) SetPlaceholder(ctx context.Context, id uint64) error {
	cacheKey := c.GetSysUserExampleCacheKey(id)
	return c.cache.SetCacheWithNotFound(ctx, cacheKey)
}

// SetPlaceholderByKey 为自定义 key 设置占位符
func (c *sysUserExampleCache) SetPlaceholderByKey(ctx context.Context, key string) error {
	cacheKey := c.GetSysUserExampleCacheKeyString(key)
	return c.cache.SetCacheWithNotFound(ctx, cacheKey)
}
...
func (c *sysUserExampleCache) IsPlaceholderErr(err error) bool {
