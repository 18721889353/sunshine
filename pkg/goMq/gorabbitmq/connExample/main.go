...
func main() {
	...
	singleConn, err := createConnectionWithRetry(ctx, "amqp://sunjianguo:jianguo123@43.143.78.234:5672/", 0)
	if err != nil {
		log.Printf("重试后仍无法创建单个连接: %v", err)
	} else {
		defer singleConn.Close()
		status := singleConn.GetConnectionStatus(ctx)
		fmt.Printf("单个连接状态: %+v\n", status)
...
func createConnectionWithRetry(ctx context.Context, url string, maxRetries int) (*gorabbitmq.Connection, error) {
