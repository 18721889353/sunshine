    ...
    <el-row :gutter="20">
      <el-col :span="12">
        <el-card class="config-card">
          <template #header><span>gRPC 连接配置</span></template>
          <el-form label-width="100px">
            <FormField v-model="form.rpcServerNames" label="gRPC 服务名" placeholder="多个用逗号分隔" />
            <FormField v-model="form.moduleName" label="模块名称" placeholder="Go 模块名" />
            <FormField v-model="form.serverName" label="服务名称" placeholder="当前服务名（mono-repo 模式）" />
...
const {
  ...
  canSubmit: (f) => f.rpcServerNames?.trim() !== '',
})
</script>
