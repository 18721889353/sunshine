<template>
  <div class="generate-page">
    <el-card class="main-card">
      <template #header>
        <span class="card-title">生成handler CRUD代码</span>
      </template>
      ...
      <el-form label-width="120px" label-position="right">
        ...
        <el-form-item label=" ">
          <div style="display:flex;align-items:center;gap:8px;">
            <el-switch v-model="form.webType" />
            <span>protobuf类型</span>
            <el-tooltip placement="right">
              <template #content>
                如果关闭，表示生成的handler CRUD代码只适合"①基于sql创建的web服务"的项目代码，<br/>如果开启，表示生成的handler CRUD代码只适合"③基于protobuf创建的web服务"的项目代码。
              </template>
              <el-icon style="color:#c0c4cc;cursor:pointer;"><QuestionFilled /></el-icon>
            </el-tooltip>
          </div>
...
const {
  ...
  buildArgs: (f, { dbDriver, dsn, tables }) => {
    ...
    if (f.webType) args.push('--web-type=true')
    return args
  },
  canSubmit: (f, { dbDriver, dsn, tables }) => dbDriver && dsn && tables.length > 0 && f.moduleName?.trim() !== '',
})
</script>
