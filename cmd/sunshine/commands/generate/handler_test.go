...
// TestHandlerGenerator_generateCode 端到端测试：基于 cp_dealer_api_log 表生成 Handler CRUD 代码
func TestHandlerGenerator_generateCode(t *testing.T) {
	dsn := "root:jianguo123@tcp(43.143.78.234:3306)/coupon_platform"
	tableName := "cp_dealer_api_log"

	tmpOut := filepath.Join(os.TempDir(), "sunshine-test", "handler-gen")
	os.RemoveAll(tmpOut)
	testDataDir := filepath.Join("testdata", "handler-gen")
