...
// TestHandlerPbGenerator_generateCode 端到端测试：基于 cp_dealer_api_log 表生成 Handler + Protobuf CRUD 代码
func TestHandlerPbGenerator_generateCode(t *testing.T) {
	dsn := "root:jianguo123@tcp(43.143.78.234:3306)/coupon_platform"
	tableName := "cp_dealer_api_log"

	tmpOut := filepath.Join(os.TempDir(), "sunshine-test", "handler-pb-gen")
	os.RemoveAll(tmpOut)
	testDataDir := filepath.Join("testdata", "handler-pb-gen")
	...
	t.Logf("解析结果: model=%d chars, dao=%d chars, handler=%d chars, proto=%d chars, crudInfo=%d chars, tableName=%s",
		len(codes["model"]), len(codes["dao"]), len(codes["handler"]), len(codes["proto"]), len(codes["crudInfo"]), codes["tableName"])

	gen := &handlerPbGenerator{
