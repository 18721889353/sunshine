...
func GRPCAndHTTPPbCommand() *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		Long:  "根据 protobuf 文件生成 gRPC+HTTP 混合服务代码。",
		Example: color.HiBlackString(`  # =====================================================================
  # 基本用法：根据 protobuf 文件生成 gRPC+HTTP 混合服务代码
  # 执行后会生成: internal/server/（http.go + grpc.go）、internal/service/、internal/routers/ 等
  # =====================================================================
  ...
  #   --out             输出目录（可选，默认 ./serverName_grpc-http-pb_<时间戳>）
`),
		SilenceErrors: true,
		SilenceUsage:  true,
	...
	cmd.Flags().StringVarP(&outPath, "out", "o", "", "输出目录，默认为 ./serverName_grpc-http-pb_<时间戳>")
...
func (g *httpAndGRPCPbGenerator) generateCode() error {
	...
	subDirs := []string{
		"cmd/serverNameExample_grpcHttpPbExample", "sunshine/configs",
...
func (g *httpAndGRPCPbGenerator) addFields(r replacer.Replacer) []replacer.Field {
	...
	fields = append(fields, []replacer.Field{
		...
		{
			Old: `"github.com/18721889353/sunshine/pkg/gin/prof"`,
			New: "",
		},
		{
