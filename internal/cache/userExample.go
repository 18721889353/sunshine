...
const (
	// UserExampleCachePrefixKey UserExample 缓存数据的 Redis key 前缀
	// 实际存储格式：data:userExample::{id}
...
// UserExampleCache UserExample 业务缓存接口
//
// 设计思路：
//   - 直接嵌入 dao.Cache[model.UserExample]，自动继承所有缓存方法
//   - 如需添加自定义缓存方法，在此接口中声明并在 userExampleCache 中实现
//
// 继承的方法包括：
...
// userExampleCache UserExample 缓存实现
//
// 设计思路：
//   - 嵌入 *dao.CacheAdapter[model.UserExample]，自动获得所有缓存能力
//   - 无需手写任何透传方法，直接使用泛型适配器的方法
//   - 当需要自定义缓存逻辑时，在此结构体上添加新方法
type userExampleCache struct {
	*dao.CacheAdapter[model.UserExample]
}
...
//
// 返回值：
//   - UserExampleCache: 缓存接口实例，传入 nil 或不支持的类型时返回 nil
//
// 创建流程：
